...
from motion_planner import TrajectoryGenerator
...
home_quat = utils._rotation_to_quaternion(robot.forward_kinematics(home_joints)[-1][:3,:3])
...
above_bin_quat = utils._rotation_to_quaternion(above_bin_ee)
...
above_marker_1_ee = np.eye(4)
above_marker_1_ee[:3,3] = pen_grab_pose.translation + np.array([0,0,config.PEN_LENGTH])
...
above_marker_1_quat = utils._rotation_to_quaternion(pen_grab_pose.rotation)

home_to_am1 = tg.generate_straight_line(robot.forward_kinematics(home_joints)[:3,3], above_marker_1_ee[:3,3], fa.get_joints(), robot.forward_kinematics(home_joints)[:3,:3], above_marker_1_ee[:3,:3],5000)
...
# ee_points = utils.whitebfa = FrankaArm()oard_2d_to_3d(robot, discretized_points, whiteboard_center_joints, whiteboard_T)
...
wb_translated_ee[:4,3] = whiteboard_center_ee @ wb_translation
...
wb_to_home = utils._slerp(whiteboard_center_quat, home_quat, five_seconds)

# home --> above bin --> open gripper --> home
home_to_ab = utils._slerp(home_quat, above_bin_quat, five_seconds)
ab_to_home = utils._slerp(above_bin_quat, home_quat, five_seconds)
